 ...
 * List API keys
 *
 * List all API keys for your organization, including each key's name, prefix, expiration, and per-key credit usage. The secret value of a key is only shown once at creation and is never returned here.
 *
 ...
 * Get API key
 *
 * Return details about an API key, including its name, prefix, expiration, and per-key credit usage. Acts on the key that authenticates this request unless target is OTHER.
 *
 * <span>⚡ <strong>Rate limit:</strong> 30 requests per 1 minute</span>
 *
 * <span>💰 <strong>Cost:</strong> FREE! No credits are charged for this API.&nbsp;<span title="Pricing shown is default pricing. Actual pricing may vary.">ⓘ</span></span>
 ...
 * Revoke API key
 *
 * Permanently revoke an API key. Acts on the key that authenticates this request unless target is OTHER. The key stops working immediately and cannot be restored. Create a replacement key from the dashboard before revoking a key that is still in use.
 *
 * <span>⚡ <strong>Rate limit:</strong> 30 requests per 1 minute</span>
 *
 * <span>💰 <strong>Cost:</strong> FREE! No credits are charged for this API.&nbsp;<span title="Pricing shown is default pricing. Actual pricing may vary.">ⓘ</span></span>
 ...
 * Reset API key usage
 *
 * Reset an API key's lifetime usage back to zero. Acts on the key that authenticates this request unless target is OTHER. Use this after raising a key's limit, or to give a key a fresh allowance without changing its ceiling.
 *
 * <span>⚡ <strong>Rate limit:</strong> 30 requests per 1 minute</span>
 *
 * <span>💰 <strong>Cost:</strong> FREE! No credits are charged for this API.&nbsp;<span title="Pricing shown is default pricing. Actual pricing may vary.">ⓘ</span></span>
 ...
 *
 * Send a one-time verification code to a work email to start a cardless API trial. Optional firstName, lastName, and companyName are stored for account creation after verify-otp.
 *
